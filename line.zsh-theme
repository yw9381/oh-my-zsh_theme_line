...
# ---------------- Get Screen Info --------------- #
local SCREEN_ID=`echo $STY|cut -d "." -f 1`
local SCREEN_NAME=`echo $STY|cut -d "." -f 2`
if [ ! "${SCREEN_ID}" = "" ];then
    local SCREEN_INFO='(${COLOR_WHITE}${SCREEN_ID}${COLOR_RESET}.${COLOR_GREEN}${SCREEN_NAME}${COLOR_RESET})'
else
    local SCREEN_INFO=''
fi
# ---------------- Set Format --------------- #
# Format For PROMPT
PROMPT="${USER_HOST}${SCREEN_INFO}: ${CURRENT_DIR} ${GIT_BRANCH}
${RETURN_STATUS} "
